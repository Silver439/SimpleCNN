...
int main()
{
	...
	if (conf1 > conf2) {
		cout << "对不起，你有" << conf1 * 100 << "%" << "的概率不是人。" << endl;
	}
	else {
		cout << "恭喜！你有" << conf2 * 100 << "%" << "的概率是个人呢！" << endl;
	}
}
	...
	int st = cp.stride;
		...
		for (int i = 1; i <= cha; i++)
		{
			for (int j = 1; j <= row; j++)
			{
				for (int k = 1; k <= col; k++)
				{
					pd_inp[(i - 1) * (row + 2) * (col + 2) + j * (col + 2) + k] = inp[n++];
				}
			}
		}
		delete[] inp;
		...
		{
			int forword_num = i * sk * sk * cp.in_channels;
			for (int j = 1; j <= cha; j++)
			{
				int fn = forword_num + (j - 1) * sk * sk;
				for (int k = 1; k <= row; k += cp.stride)
				{
					for (int l = 1; l <= col; l += cp.stride)
					{
						int fn2 = (j - 1) * (row + 2) * (col + 2) + k * (col + 2) + l;
						res[i * row * col / st / st + (k - 1) * col / st / st + (l - 1) / st] += pd_inp[fn2] * cp.p_weight[fn + 4] +
